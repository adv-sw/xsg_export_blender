...
class Export_Material:
	...
	def Write(exp, mtl):
		...
		if mtl.node_tree == None:
			...
			if principled_bsdf != None:
				...
				# Approximate XSG reflectivity ...
				metallic_input = principled_bsdf.inputs['Metallic']
				metallic_tint = principled_bsdf.inputs['Subsurface Color']

				#if metallic_input.is_linked == False :
				color = metallic_tint.default_value
				color[0] = color[0] * metallic_input.default_value;
				color[1] = color[1] * metallic_input.default_value;
				color[2] = color[2] * metallic_input.default_value;
				skip = (color[0] == 0) and (color[1] == 0) and (color[2] == 0)
				
				if skip == False:
					Export_Material.Part_Write_Ref(exp, mtl, "reflection", color)
	...
	def Part_Write_Ref(exp, mtl, inf_id, value) :
		...
		exp.file.Write('<input ')
		exp.file.Write('color="{:f} {:f} {:f}"'.format(value[0], value[1], value[2]), Indent=False)
		exp.file.Write(' ', Indent=False)
		exp.file.Write('/>\n', Indent=False)
